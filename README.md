...
**Вкладки:** 📊 Анализ · ⚖️ Сравнение · ⭐ Избранное · 💎 Идеи (подборка по S&P 500).
...
Курируемая подборка акций S&P 500 (5 пресетов + фильтры) на предпосчитанном
снимке. Обновить снимок:

```bash
python3 scripts/fetch_sp500.py       # список тикеров (разово)
python3 scripts/build_snapshot.py    # снимок ~15-25 мин, затем git push
```
...
pages/            вкладки Streamlit (Анализ, Сравнение, Избранное, Идеи)
core/             чистая логика: data, dcf, compare, favorites, screener
ui/               отрисовка: theme, sidebar, components
scripts/          fetch_sp500.py, build_snapshot.py
data/             sp500.txt, sp500_snapshot.json
